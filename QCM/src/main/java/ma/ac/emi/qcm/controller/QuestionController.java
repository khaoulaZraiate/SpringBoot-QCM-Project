...
public class QuestionController {
	...
	public String getQuestions(Model model,
			@PathVariable("qcm_id") long qcmId, @Valid QCM qcm,
			@RequestParam(name = "page", defaultValue = "0") int page,
			@RequestParam(name = "size", defaultValue = "4") int size) {
		try {

			if (qcmId >= 0) {
				qcm = qcmRepository.getOne(qcmId);
				
				List<Question>  questions=questionService.generateQuestionString(qcm.getFormateur(), qcm.getMatiere());
